class DeterminantSolver:
    ...
    def det_cramer(self, matrix):
        ...
        for j in range(n):
            ...
            steps.append(f"Cofactor para a[0][{j}]: {cofactor}, producto: {matrix[0][j]} * {cofactor} = {matrix[0][j] * cofactor}")
        self.steps = steps
        return det

    def det_sarrus(self, matrix):
        """
        ...
        steps = [
            f"Diagonal principal: {a[0]}*{b[1]}*{c[2]} + {a[1]}*{b[2]}*{c[0]} + {a[2]}*{b[0]}*{c[1]} = {diag_principal}",
            f"Diagonal secundaria: {a[2]}*{b[1]}*{c[0]} + {a[1]}*{b[0]}*{c[2]} + {a[0]}*{b[2]}*{c[1]} = {diag_secundaria}",
            f"Determinante: {diag_principal} - {diag_secundaria} = {det}"
        ]
        self.steps = steps
        return det

    def det_cofactor(self, matrix):
        """
        ...
        for j in range(n):
            minor = self.get_minor(matrix, 0, j)
            cofactor = ((-1) ** (0 + j)) * self.det_cofactor(minor) if n > 1 else matrix[0][j]
            det += matrix[0][j] * cofactor
            steps.append(f"Cofactor C[0][{j}] = {cofactor}, producto: {matrix[0][j]} * {cofactor} = {matrix[0][j] * cofactor}")
        self.steps = steps
        return det
