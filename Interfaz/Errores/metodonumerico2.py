...
from Models.solverNewtonRaphson import metodo_newton_raphson
from Models.solverSecante import metodo_secante
from Models.solverBiseccion import metodo_biseccion
from Models.solverRegla_Falsa import metodo_regla_falsa
...
class NumericalMethodsCalculator(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        self.metodo_menu = ctk.CTkOptionMenu(
            metodo_frame,
            values=["Bisección", "Regla Falsa","Newton-Raphson", "Secante"],
            width=140,
        ...
        # Cambiar etiquetas de a y b a x0 y x1
        self.a_entry_label = ctk.CTkLabel(entradas, text="x0:", text_color=COLOR_TEXTO)
        self.a_entry_label.grid(row=1, column=0, padx=10, pady=5, sticky="w")
        self.b_entry_label = ctk.CTkLabel(entradas, text="x1:", text_color=COLOR_TEXTO)
        self.b_entry_label.grid(row=1, column=2, padx=10, pady=5, sticky="w")
        ...
        self.procedimiento_frame = ctk.CTkFrame(
        self.resultados_frame, fg_color=COLOR_ENTRADA, corner_radius=5)
        self.procedimiento_frame.pack(fill="both", expand=True, padx=10, pady=(5, 10))  # Quita height y pack_propagate
        # Crear un tk.Text con scrollbars
        self.info_textbox = tk.Text(
            self.procedimiento_frame, wrap="word", bg=COLOR_ENTRADA, fg=COLOR_TEXTO,
            font=("Arial", 12), state="disabled", height=10  # height en líneas, ajusta si es necesario
        )
        v_scroll = tk.Scrollbar(self.procedimiento_frame, orient="vertical", command=self.info_textbox.yview)
        h_scroll = tk.Scrollbar(self.procedimiento_frame, orient="horizontal", command=self.info_textbox.xview)
        self.info_textbox.configure(yscrollcommand=v_scroll.set, xscrollcommand=h_scroll.set)
        # Empaquetar
        self.info_textbox.pack(side="left", fill="both", expand=True)
        v_scroll.pack(side="right", fill="y")
        h_scroll.pack(side="bottom", fill="x")
        self.info_textbox.insert("1.0", "Ingrese los datos y presione 'Ejecutar Método'.")
    ...
    def cambiar_metodo(self, metodo):
        if metodo == "Newton-Raphson":
            self.b_entry_label.grid_remove()
            self.b_entry.grid_remove()
        elif metodo == "Secante":
            self.b_entry_label.grid()
            self.b_entry.grid()
        else:  # Bisección o Regla Falsa
            self.b_entry_label.grid()
            self.b_entry.grid()
    ...
    def ejecutar_metodo(self):
        ...
        try:
            ...
            x1 = float(self.b_entry.get()) if metodo in ["Bisección", "Regla Falsa", "Secante"] else None
            tol = float(self.tol_entry.get())
        except ValueError:
            messagebox.showerror("Error", "x0, x1 (si aplica) y tolerancia deben ser números válidos.")
            return

        if metodo in ["Bisección", "Regla Falsa"]:
            if x0 >= x1:
                messagebox.showerror("Error", "x0 debe ser menor que x1.")
                return
            try:
                fa = safe_eval(f_expr, x0)
                fb = safe_eval(f_expr, x1)
            except Exception as e:
                messagebox.showerror("Error de Función", f"{e}")
                return
            if fa * fb >= 0:
                messagebox.showerror("Error", "f(x0) y f(x1) deben tener signos opuestos.")
                return
        elif metodo == "Newton-Raphson":
            pass  # No necesita validaciones adicionales, ya que calcula derivada automáticamente
        elif metodo == "Secante":
            pass  # No hay validaciones adicionales específicas
        ...
        try:
            ...
            if metodo == "Bisección":
                ...
                xr, iter_count, error_rel, historial, paso_a_paso_str = metodo_newton_raphson(
                    f_expr, x0, tol  # Solo pasa f_expr y x0, sin df_expr
                )
            elif metodo == "Secante":
                xr, iter_count, error_rel, historial, paso_a_paso_str = metodo_secante(
                    f_expr, x0, x1, tol
                )
            else:
            ...
            intervalo_part = "Intervalo inicial" if metodo in ["Bisección", "Regla Falsa"] else "Puntos iniciales"
            info = (
                f"Método: {metodo} | f(x) = {f_expr}\n"
                f"{intervalo_part}: [{x0}, {x1}] | Tolerancia: {tol}\n"
