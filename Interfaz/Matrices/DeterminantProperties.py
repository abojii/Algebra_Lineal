...
class PropiedadesDeterminanteApp(ctk.CTkFrame):
    ...
    def setup_ui(self):
        ...
        label_a = ctk.CTkLabel(self.frame_matrices, text="Matriz A:", font=ctk.CTkFont(weight="bold"), text_color=COLOR_TEXT)
        ...
        label_b = ctk.CTkLabel(self.frame_matrices, text="Matriz B (solo para propiedad 5):", font=ctk.CTkFont(weight="bold"), text_color=COLOR_TEXT)
        ...
        # === RESULTADOS ===
        self.result_box = ctk.CTkTextbox(self, wrap="word", fg_color=COLOR_ENTRY, font=ctk.CTkFont(size=12), text_color=COLOR_TEXT)
    ...
    def generar_campos(self):
        """Genera campos de entrada para matrices A y B según n."""
        try:
            ...
            self.result_box.insert("1.0", f"Campos generados para matrices A y B de {n}x{n}.\n")
    ...
    def verificar_propiedad(self):
        """Muestra explicación de la propiedad seleccionada."""
        propiedad = self.propiedad_var.get()
        self.result_box.delete("1.0", "end")

        explicaciones = {
            "1️⃣ Fila o columna nula ⇒ det(A)=0":
                "✅ Si una fila o columna tiene todos sus elementos iguales a 0, el determinante es 0.",
            "2️⃣ Filas o columnas iguales/proporcionales ⇒ det(A)=0":
                "✅ Si dos filas o columnas son idénticas o proporcionales, el determinante es 0.",
            "3️⃣ Intercambio de filas ⇒ cambia signo":
                "✅ Si se intercambian dos filas, el valor del determinante cambia de signo.",
            "4️⃣ Multiplicación de fila por escalar ⇒ det(kA)=k·det(A)":
                "✅ Multiplicar una fila por un escalar k multiplica el determinante por el mismo k.",
            "5️⃣ Propiedad multiplicativa ⇒ det(AB)=det(A)·det(B)":
                "✅ El determinante del producto de dos matrices equivale al producto de sus determinantes.",
            "6️⃣ Suma de múltiplo de una fila a otra ⇒ det(A) no cambia":
                "✅ Si se suma un múltiplo escalar de una fila (o columna) a otra, el determinante no cambia."
        }

        if propiedad in explicaciones:
            self.result_box.insert("1.0", f"🔍 Propiedad seleccionada:\n{propiedad}\n\n")
            self.result_box.insert("end", explicaciones[propiedad])
            self.result_box.insert("end", "\n\n💡 Llena las matrices A y B según el caso para visualizar su efecto.")
            if propiedad == "6️⃣ Suma de múltiplo de una fila a otra ⇒ det(A) no cambia":
                self.result_box.insert("end", "\n\n⚠️ Recuerda: en general det(A + B) ≠ det(A) + det(B).")
        else:
            messagebox.showwarning("Atención", "Selecciona una propiedad primero.")
