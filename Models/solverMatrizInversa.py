...
def gauss_jordan_inverse(A):
    ...
    Si no es invertible, retorna None y los pasos hasta el error.
    """
    n = len(A)
    ...
    for i in range(n):
        ...
        for j in range(2 * n):
            augmented[i][j] /= pivot
        steps.append(f"Dividir fila {i+1} por {pivot:.4f} para hacer el pivote 1:")
        steps.append(matrix_to_string(augmented))
        ...
        for k in range(n):
            if k != i:
                ...
                steps.append(f"Eliminar entrada en fila {k+1} usando fila {i+1} (factor {factor:.4f}):")
                steps.append(matrix_to_string(augmented))
...
def matrix_to_string(matrix):
    """
    Convierte una matriz en string para mostrar en la interfaz.
    """
